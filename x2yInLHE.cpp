// c++ -o x2yInLHE `root-config --glibs --cflags` -lm x2yInLHE.cpp
...
int main(int argc, char ** argv) {
  if(argc < 5) {
   std::cout << "Usage:   " << argv[0] << " input.lhe output.lhe xId yId" << std::endl ;
   return -1;
  }
  ...
  int xId_n = (abs(xId)+1) * ( - abs(xId)/xId); //---- neutrino
  int yId_n = (abs(yId)+1) * ( - abs(yId)/yId); //---- neutrino
...
//  float k2 = 0.000510998928 * 0.000510998928 - 1.77682 * 1.77682 ; // GeV -3.15708905128
  ...
  if (yId != 15 && yId != -15) k2 = 0;
 
  std::cout << " xId = " << xId << std::endl;
  std::cout << " yId = " << yId << std::endl;
  ...
  while (reader.readEvent ()) {
   ...
   for (int iPart = 0 ; iPart < reader.hepeup.IDUP.size (); ++iPart) {
    ...
    if (reader.hepeup.ISTUP.at (iPart) == 1) {
     if ( (reader.hepeup.IDUP.at (iPart)) == xId) {    //  xId
      ...
      reader.hepeup.IDUP.at (iPart) = yId ;    //  yId
     }
     if (reader.hepeup.IDUP.at (iPart) == xId_n) reader.hepeup.IDUP.at (iPart) = yId_n ;
    } // outgoing particles
   } // loop over particles in the event
